...
class db_apt
{
    public static $error_call = '';
    public static $allow_regx = '#^([a-z0-9\(\)\._=\-\+\*\`\s\'\",]+)$#i';
    public $table = '';
    public $primary = 'id';
    public $select = '*';
    public $sql = '';
    ...
    public function fastpaging()
    {
        ...
        $this->pager = load::classes('lib.util.pager', TEA_PATH, ['total'=>$this->num, 'pagesize'=>$this->page_size]);
    }
    ...
    public function getone($field = '', $cache_id = '')
    {
        $this->limit('1');
    ...
    public function getall($cache_id = '')
    {
        if ($this->is_execute == 0) {
            $this->exeucte();
        }
