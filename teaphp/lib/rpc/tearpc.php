...
class tearpc
{
    ...
    public function add($function, $class = '', $name = '')
    {
        //如果函数为空
        if (empty($function)) {
            $this->errormsg(301, 'TEA RPC Error - Functions can not be empty!');
        }
        //如果函数为一个类
        ...
        if (is_string($function)) {
            if (empty($class)) {
                $func = $function;
                if (!is_callable($func)) {
                    $this->errormsg(302, "TEA RPC Error - Function $function does not exists!");
                }
            } elseif (is_object($class)) {
                $func = [&$class, $function];
                if (!is_callable($func, true, $call_func)) {
                    $this->errormsg(303, "TEA RPC Error - Function $call_func does not exists!");
                }
            } elseif (is_string($class)) {
                $func = [$class, $function];
                if (!is_callable($func, true, $call_func)) {
                    $this->errormsg(304, "TEA RPC Error - Function $call_func does not exists!");
                }
            }
    ...
    public function set_gzip($enablegzip)
    {
        $this->gzip = $enablegzip;
    }

    public function out_header()
    {
    ...
    public function out_gzip($buffer)
    {
        $len = strlen($buffer);
