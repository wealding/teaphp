...
class saemplate
{
    ...
    public function __construct()
    {
        if (empty($this->compile_dir)) {
            $this->compile_dir = 'saemc://saemplate_c';
        }
    }
    ...
    public function set_view($view)
    {
        ...
        $this->compile_file = $this->compile_dir.DIRECTORY_SEPARATOR.str_replace(['/', '\\'], ',', $view).'.php';
        $this->compile_time = $this->compile_dir.DIRECTORY_SEPARATOR.str_replace(['/', '\\'], ',', $view).'.time';
    ...
    protected function _compile($view = null)
    {
        ...
        if (false === @file_put_contents($this->compile_file, $data)) {
            debug::error('<font color=red>Sae</font>mplate Error', "$this->compile_file file is not writable.Check your memcache setting!");
        }
        if (false === @file_put_contents($this->compile_time, time())) {
            debug::error('<font color=red>Sae</font>mplate Error', "$this->compile_time file is not writable.");
        }
