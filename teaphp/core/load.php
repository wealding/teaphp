...
class load
{
    ...
    {
        $postfix = ['.php', '.func.php', '.class.php'];
    ...
    {
        foreach ([TEA_PATH, TEA_PATH.'/core', TEA_PATH.'/lib', APP_PATH, APP_PATH.'/controller', APP_PATH.'/model', APP_PATH.'/lib'] as $path) {
            $postfix = ['.php', '.func.php', '.class.php'];
            foreach ($postfix as $pfix) {
                if (file_exists($path.DIRECTORY_SEPARATOR.$name.$pfix)) {
                    return $path.DIRECTORY_SEPARATOR.$name.$pfix;
